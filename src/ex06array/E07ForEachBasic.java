...
/*
 ...
 그러므로 참고의 목적으로만 사용해야하고, 값의 변경은 기본적으로 허용하지않는다
 	형식]
 		for(자료형 차조변수 : 배열의이름){
 		실행문장;
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		/*
		 배열 arr의 요소를 하나씩 참조하여 출력하고, 더이상 참조할 요소가 없을대 for문이 자동으로 
		 종료된다
		 */
		sum=0;
		for(int e : arr) {
		...
		for(int b : arr) {
			b++;
			System.out.print(b+" ");//+1 증가된값
		}
		System.out.println();
		System.out.println("----------------------");
		System.out.println("배열arr의요소 재출력시키기");
		for(int c : arr) {
			System.out.print(c+ " ");
			/*
		...
		/*
		 ...
		 우선 하나의 행(1차원배열)을 가져와서 각 요소를 하나씩 참조하여
		 출력한다
		 */
		System.out.println("----------------------");
