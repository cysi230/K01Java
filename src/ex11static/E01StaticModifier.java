...
/*
 ...
 -멤버변수앞에 static지정자가 붙으면 정젹변수가 된다
 -정적변수는 JVM(자바가상머신)에 의해 프로그램에 로드되는
 시점에 초기화된다
 
 -main()메소드가 실행되기전에 이미 초기화 되므로 main메소드
...
class MyStatic{
	...
	int instanceVar;
	/*
	 정적멤버변수]객체를 생성할 필요없이 클래스명으로 바로접근이가능하다
	 		  프고르램시작시 이미 메소드영역에 초기화된다
	 */
	...
	/*
	 정적멤버 메소드]정적메소드 안에서는 인스턴스 멤버를 사용 할 수없다.
	 			정적멤버만 접근 가능하다
	 			(※main메소드에서는 static으로 선언된 메소드만 호출할수 있었음을 기억하자)
	 */
	static void staticMethod() {
		//System.out.println("instatnceVar= "+instanceVar);//오류 static-static끼리만
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 일반적인 멤버변수에 접근하는 방법 - >
		 MyStatic 클래스의 객체를 생성한후 참조변수를 통해 접근한다
		 */
		...
		/*
		 static(정적) 멤버변수 접근방법 
		 방법1: 클래스명을 통해서 접근한다.객체생성없이 바로접근이 가능하다.
		 */
		MyStatic.staticVar =200; //static
		System.out.println("MyStatic.instanceVar="+ myStatic.staticVar);
		
		/*
		 방법2: 일반적인 멤버변수처럼 참조변수를 통해 접근하는것도가능하다
		 단, 이경우는 정적변수를 일반적인 멤버변수로 착각할수 있으므로 자바에서 권장하지않는다
		 */
		myStatic.staticVar =300; //변수
		System.out.println("MyStatic.instanceVar="+ myStatic.staticVar);
		...
		/*
		 정적 멤버변수는 각각의 객체마다 멤버변수가 존재하징낳고, 메소드영역에
		 하나만 생성되어 공유되므로, 아래 대입문장은 하나의 변수에 값이 할당되게
		 된다. 따라서, 결과는 둘다 900이출력된다.
		 */
