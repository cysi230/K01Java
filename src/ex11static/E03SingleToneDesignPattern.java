...
/*
 ...
 사용하고자 할때 사용하는 패턴이다.
	...
	4. 위 방법을 통해 객체를 생성하면 메모리에 딱하나만 만들어지게된다
 */
class NoSingleTone{
	int instVar;
	public NoSingleTone() {
...
class SingleTone{
	...
	/*
	 생성자를 private으로 선언하면 클래스외부에서는 호출할수 없으므로
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/*
		 일반적인 방식의 객체생성이므로 객체를 생성할때마다 새로운 참조값(주소값)을 할당받는
		 인스턴스가 된다
		 */
		...
		//생성자가 private이므로 외부에서는 접근이 불가능
//		SingleTone st1 = new NoSingleTone();//오류
		
		/*
		 생성자를 접근 할 수 없는 대신 getIntance() 메소드를 통해서만 
		 객체의 참조값을 반환 받을 수있다. 반환받을때는 항상 같은 객체를 
		 받환받게된다
		 */
