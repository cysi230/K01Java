...
public class DBConnect {
	...
	try {
		...
		if(con!=null) {
			...
			/*
			 3]쿼리문
			 : 쿼리문을 작성할때 주의할점은 줄바꿈 할때 앞뒤로
			 스페이스를 삽입 하는것이 좋다. 그렇지않으면 문장이
			 이어지게 되어 SyntaxError가 발생하게된다
			 */
			...
			/*
			 Statement객체를 사용하여 Oracle로 작성된 쿼리문을 전송하고
			 실행한다 실행후 결과를 반환하면 ResultSet객체를 통해받는다
			 */
			...
			while(rs.next()) {
				/*
				 ...
				 콘솔에 내용을 출력한다
				 getXXXX()계열의 함수는 문자, 숫자, 날짜를 사용하기 위해
