...
/*
 ...
 - 자바에서 인스턴스의 내용비교를 위해 비교연산자(==)를 사용하는것은
 	단순히 참조값에 대한 비교이므로 실제 내용에 대한 비교는 이루어지지않는다
 - 인스턴스간의 내용 비교를 위해서는  equlas()메소드를 각 클래스에 
 	맞게 적절히 오버라이딩하여 사용해야한다
 - String과 같은 기본클래스는 별도의 오버라이딩 없어도 내용비교가 가능하다
 */
class IntNumber{
	int num;
