...
/*
 TreeSet<T> 컬렉션
 - 트리라는 자료구조로 기반으로 한 Set컬렉션이다
 - Set의 기본적인 특성은 동일하나, Tree알고리즘을 통해 
 	정렬되어 저장된다, 정렬의 기준은 개발자가 직접 정의할수있다
 - 정렬의 기준으로 정하기 위해 CompareTo<T> 인터페이스를 구현해야한다
 */
class MyString implements Comparable<MyString>{
	...
	/*
	정렬의 기준을 위해 오버라이딩한 메소드로 가나다와 같은 사전순,
...
public class Ex04TreeSetMain {
	...
	public static void main(String[] args) {
		...
		tree1.add(2);
		tree1.add(3);
		System.out.println(tree1.add(2));//Set은 중복저장허용안됨
		
		//중복이 제거되어 4개만 저장됨
		System.out.println("저장된 데이터 수"+ tree1.size()+"개");//4게
		...
		/*
		 - 문자열의 길이가 장랼의 기준일때는 아래 객체중 7글자인 KOSMOXX는 1개만 삽입된다
		 - 문자열을 compareTo()를 통해 비교하여 정렬할때는 모든 문자열이 Set에 삽입된다
		 */
