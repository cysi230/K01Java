...
public class Ex04HashSet1 {
	public static void main(String[] args) {
		...
		int number = 100; //<-Integer 객체로 Boxing 처리후 저장됨
		 ...
		 */
		System.out.println(set.add(strObject1));//true변환
		set.add(strObject2);
		set.add(dateObject);
		...
		/*
		 ...
		 : 직접 정의한 클래스인 경우 hashCode , equals 메소드를 오버라이딩 하지않으면 중복저장이 허용된다
		 아래의 경우 저장에 성공하여 true가 반환된다
		 */
		Teacher teacher2 = new Teacher("김봉두", 55, "체육");
		System.out.println(set.add(teacher2) ? "성공" : "실패");//성공
		System.out.println("[중복(teacher2) 저장전 객체수]"+ set.size());//6개
		...
		while(itr.hasNext()) {
			...
			if(object instanceof String) {
				...
				System.out.println("Integar타입"+object);
			}
		...
		//1-7] 존재여부확인
		System.out.println(set.contains(strObject1) ? "strObject1있다" : "strObject없다");
