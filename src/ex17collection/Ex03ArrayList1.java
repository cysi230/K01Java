...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 : ArrayList<T>와 같은 컬렉션은 제네릭 기반으로 하지만
		 아래와 같이 타입매개변수를 생략하면 모든 객체를 저장 할 수 있는
...
//		ArrayList<Object> list = new ArrayList();
		...
		list.add("트와이스");
		list.add(3, "워너원");
//		list.add(5, "소방차");// [에러발생] : 인덱스를 전너띌수 없다
		 ...
		 */
		System.out.println(list.add("트와이스")? "중복저정됨":"중복저장안됨");
		System.out.println("중복 저장 후 객체수: "+list.size());
		
		/*
		 1-3]리스트 컬렉션 출력하기(for. 확장for문)
		 : 컬렉션을 다룰때는 주로 for(for each)문을 주로 사용한다
		 */
		System.out.println("\n[일반 for문 사용]");
		...
		/*
		 Iterator
		 1. 컬렉션에 저장된 내용을 Iterator에게 알려주기위해 객체를 생성한다
		 2. hasNext()로 반환할 객체가 있는지 검사하고, next()로 해당
		 	객체를 반환한다
		 3.모든 객체를 반환했다면 false로 반환한다.
		 */
		System.out.println("\n[반복자(Iterator) 사용]");
		...
		/*
		 1-6]포함여부확인
		 contains(객체): 찾고자하는 객체가 있는 경우 true를 반환한다.
		 			위치한 인덱스까지는 확인 할 수 없다.
		 */
		System.out.println(list.contains("빅뱅")? "빅뱅있음": "빅뱅없음");
		System.out.println(list.contains("블랙핑크")? "블랙핑크있음": "블릭핑크없음");
		
		/*
		 1-7]삭제
		 방법1: 인덱스로 삭제하기
		 	removeI(index) : 인덱스를 통해서 삭제를 진행한후
		 	삭제가 완료되면 해당 객체를 반환한다. 삭제후에는
		 	인덱스가 자동으로 재부여된다. 삭제 성공시 해당객체를
		 	반환 하므로 어떤객체가 삭제되었는지 확인 가능하다.
		 */
		Object obj = list.remove(2);
		System.out.println("삭제된 객체: "+ obj);
		
		//방법2 : indexOf()를 통해 인덱스를 찾은 후에 삭제
