...
public class Human {
	...
	//행동을 표현하는 멤버메소드: 걷기,생각하다,먹다,객체의 현태상태 표현
	void walk() {
		//에너지를 1 차감한후 음수인지 음수이면 0으로 고정한다
		energy--;
		if(energy<0) {
			energy=0;//-면 0으로고정 내려가지않는다
			System.out.println("[walk]에너지는 0이 최소입니다.");
			System.out.println("0으로 고정됩니다");
		}
		else {
			System.out.println("[walk]에너지는 1이 감소하였습니다.");
		}
	}
	void thinking() {
		energy -=2;
	...
	void eat() {
		//에너지가 2증가 시킨후 10을 초과하면 10으로 고정한다
		energy +=2;
		if(energy>10) {
			energy=10;
