...
/*
 ...
 멤버메소드와 완전히 동일한 모양으로 제정의하는것을 말한다
 오버로딩(Overloading)과 다른점은 매개변수의 갯수,
 반환타입 등이 완전히 똑같은 형태로정의한다는 것이다.
 하위클래스에서 오버라이딩한 메소드는 상위클래스의 메소드를 가리게되므로 
 항상 최하위클래스에서 오버라이딩한 메소드가 호출되게된다
 */
...
class BaseSpeaker extends Speaker{
	...
	/*
	 Override : " 어노테니션"이라 부르고 오버라이딩된 메소드에 추가하여 컴파일러에게 알려주는 역할을한다.
	 */
	@Override
	public void showState() {
		/*
...
public class E06Overriding{
	public static void main(String[] args) {
		...
		System.out.println();
		/*
		 [[[[부모클래스의 참조변수가 자식클래스의객체를 참조할수있다]]]]]
		 이경우 접근범위는 부모클래스으로 제한 한다
		 즉 아래로출부분에서 speaker참조변수를 통해서는 setBase()메소드는 호출할수 없다
		 setBase()는 자식클래스에 정의된 멤버메소드이기떄문이다
