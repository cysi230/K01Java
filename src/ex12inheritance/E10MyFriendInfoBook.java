...
class Friend{
	...
	/*
	 간략한 정보만을 출력하는 멤버메소드로,현재 실행부가 없는 상태로정의되었다.
...
class HighFriend extends Friend{
	...
	/*
	 고딩친구의 전체정보를 출력하기 위해 부모에서 정의된 메소소드를
...
public class E10MyFriendInfoBook {
	...
	public static void main(String[] args) {
		//프로그램의 비즈니스 로직을 가진 핸들러 객체를 생성함
		FriendInfoHandler handler = new FriendInfoHandler(100);/*
										연락처 정보를 저장할 객체배열의 크기를
										100으로 초기화함
		*/
		
		//무한루프로 구성하여 사용자가 종료하는시점에 프로그램 종료됨.
		while(true) {
			//메뉴출력을 위한 메소드호출
			menuShow();
...
class FriendInfoHandler{
	//멤버변수
	/*
	 Friend탕비의 객체배열을 선언하여 하위클래스인 High, Univ객체를
	 모두저장한다 부모타입의 배열에는 하위객체 전체를 저장할수 있으므로
	 여러개의 객체를 하나의 타입으로 관리할수있다는 장점이있다.
	 */
	...
	public void addFriend(int choice) {
		...
		System.out.println("친구정보입력이 완료되었습니다");	
	}//end of addFriend
	...
	public void showAllData() {
		...
		System.out.println("==전체 정보가 출력 되었습니다==");
	}//end of showAllData
	
	//친구정보간략보기
	/*
	 1.고딩친구 정보 Friend 객체배열에 추가
	 2.High 객체가 Friend형으로 자동형변환되어 저장됨
	 3.객체배열에 저장된 객체들은 for문을 통해 순차적으로 정보출력
	 	이때 오버라이딩된 메소드를 통해 항상 해당객체의 정보를 출력할수있음
	 	참초변수의 타입에 영향을 받지않음
	 */
	...
	public void searchInfo() {
		boolean isFind =false;//검색한 정보가 있는 확인 하기위한 변수
		Scanner scan = new Scanner(System.in);
	...
	public void deleteInfo() {
		...
		/*
		 ...
		 요소를삭제한 후 빈자리를 채워넣을때 사용할 것임
		 */
		int deleteIndex= -1; //배열의 index이므로 초기값은 -1 ,ex)덧셈 0 곱셈1 배열 -1
		
		for(int i=0; i<numOfFriends ; i++) {
			if(deleteName.compareTo(myFriends[i].name)==0) {
				//요소를 삭제하기 위한 null값으로 변경
				myFriends[i]= null;
				//삭제된 요소의 index를 저장
				deleteIndex = i;
				//전체카운트를 -1차감
		...
		if(deleteIndex==-1) {
			...
			/*
			 객체배열에서 검색된 요소를 삭제한 후 입력된 위치의 바로뒤 요소를 앞으로하나씩 교환한다
			 */
