...
class Computer{
	...
	/*
	 private 접근 지정자로 선언된 멤버메소드는 외부클래스 혹은
	 상속받은 클래스에서 접근이 불가능하므로, public 으로 선언된
	 메소드를 통해서 간접 호출해야한다.
	 상속관계라면 protected로 서언된 메소드로도 호출할수있다
	 */
...
class NotebookComputer extends Computer{
	...
	public void movigCal() {
		...
		/*
		 아래 메소드는 private으로 선언 되었으므로 상속받은
		 자식클래스에서도 접근은 불가능하다.
		 */
//		KeyboardTouch();//에러발생
//		calculate();//에러발생
...
/*
 태블릿 노트북 컴퓨터: 컴퓨터의 기본기능 및 휴대이용에 기능 그리고 펜으로 드로잉
 할수있는 기능까지 추가된 클래스
 */
class TableNotebook extends NotebookComputer{
	String registPencil;
	
	public TableNotebook(String name, int initCharg, String pen) {
		super(name, initCharg);
		registPencil = pen;
	}
	//태블릿 사용을 위해 배터리양 체크 및 등록된 펜인지확인
	public void write(String penInfo) {
		...
		/*
		 A.comapareTo(B)
		 : 문자열 A와 B를 비교하여0이 반환되면 같은 문자열로 판단하고
		 -1혹은 1이 반환되면 서로다른 문자열로파단하는 String클래스에 정의된메소드
		 */
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		/*
		 배터리가 소모되어 사용할 수 없는 상태가 되는지 확인하기 위해
		 movigCal()을 여러번 호출한다.
		 */
