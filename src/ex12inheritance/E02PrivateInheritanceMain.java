...
class Account{
	/*
	 ...
	 클래스내부에서만 접근가능하다.즉 상속받은 하위 클래스에서는 직접 접근이 불가능하다
	 */
	private int money;//잔고 클래스 내부에서만 접근가능
	public Account(int init) {
		money = init;
		
	}
	/*
	 입금처리: protected 선언되었으므로 상속관계에서 접근가능
	 */
...
class SavingAccount extends Account{
	...
	/*
	 인자생성에서 부모의 생성자 메소드를 호출하기위해 super()를 사용한다
	 이대 인자가 하나인 생서자를 호출한다.
	 */
	...
	/*
	 부모클래스에 protected로 선언된 멤버메소드를 각각호출한다
	 private 멤버변수인 money직접 접근할수 없으므로 접근 가능한
	 메소드를 통해 간접적으로 접근한다
	 */
...
//		sa.showAccountMoney();
		...
		/*
		 부모객체의 참조변수를 통해서도 private 멤버는 접근이 불가ㅡㄴㅇ하다
