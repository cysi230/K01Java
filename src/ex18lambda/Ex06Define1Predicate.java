...
/*
 JAVA에서 제공하는 기본함수형 인터페이스는 다음과 같은 종류가 있다
 큰 차이점이람녀 매개변수 의 유무, 리턴값의 유무 정도이다.
 
 Predicate:매개변수 있고 리턴은 boolean값
 Consumer: 매개변수 있고 리턴값 없음
 Supplier: 매개변수없고, 리턴값있음
 Function: 매개변수있고, 리턴값없음
 */
...
public class Ex06Define1Predicate {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 람다식정의함.test()메소드를 오버라이딩하여 정의하였으므로
		 아래와같이 호출할수있다.
		 */
		...
		System.out.println("p.test(p1) =>"+pre.test(p1));//T
		System.out.println("p.test(p2) =>"+pre.test(p2));//F
		...
		//람다식 자체를 매개변수로 전달
		//(Human a)-> { a.getGender().equals("남") };
		/*
		 이와같은 람다식을 간소화하면 아래와같이 기술할수 있다.
	...
	public static double avg(Predicate<Human> ph, List<Human> li) {
		...
		for(Human h : li) {//li컬렉션의 크기만큼 반복
			...
			if(ph.test(h)==true) {
				pCount++;//인원수증가
				tCount+=h.getScore();//점수를 누적합산
			}
		}
		return(double)tCount/pCount;//평균값을 계산후 반환
	}
