...
//   		}	
//   		else {
//   			System.out.println("우측좌표지정이 잘못되었습니다");
//   		}
//   	}	
//   	public void showArea()
//   	{
//   		System.out.println("실행결과]");
//   		System.out.println("넓이: "+(ulx-lrx)*(uly-lry)); 
//   		
//   		/* 사각형의 넓이를 구하는 메소드 정의 */
//   	}  	
//}
...
class Rectangle
{
	//멤버변수
   	...
   	//디폴트 (기본)생성자는 개발자가 생성자를 정의하지않았을때만 자동으로 생성된다.
//   public Rectangle() {
//   		
//   }
   	public Rectangle(int ulx, int uly, int lrx, int lry) {
		...
		*/
   		if(isRange(ulx)==false || isRange(uly)==false){
   			System.out.println("좌측상단의 좌표가 잘못되었습니다.");
   			isTrue = false;
   		}
   		if(isRange(lrx)==false || isRange(lry)==false){
   			System.out.println("우측상단의 좌표가 잘못되었습니다.");
   			isTrue = false;
   		}
   	...
   	public void showArea()
   	{
   		/* 사각형의 넓이를 구하는 메소드 정의 */
   		if(isTrue==false) {
   			System.out.println("좌표값 오류로 넓이계산 불가능합니다");
   		return;
   		}
   		//좌표값을 통해 가로/세로길이를 구한후 넓이를 계산
   		int xLength = lrx - ulx;
   		int yLength = lry - uly;
