...
/*
 ...
 : 메인 쓰레드와 working쓰레드(개발자가 정의한 쓰레드)를
 	일컫는 것으로 메인쓰레드가 종료되어도 계속 실행되며,
 	Dead상태가 되어야 실행이 종료되는 쓰레드를 말한다
 	해당 쓰레드는 main()메소드가 종료된 이후에도 실행되는 것을 볼수있다.
 */
class YesThread extends Thread{
	
	/*
	 쓰레드 생성 이름을 지정하지 않으면 JVM이 자동으로
	 이름을 부여한다.
	 */
	public YesThread() {}
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		 ...
		 */
		YesThread yt1 = new YesThread("1st쓰래드");//생성자를 통해 이름부여
//		yt1.setName("첫번쨰쓰레드");//쓰레드의 이름을 지정하는 두번째방법
		yt1.start();
		
		YesThread yt2 = new YesThread();//이름이 지정되지 않으면 자동으로 부여됨
		yt2.setPriority(Thread.MAX_PRIORITY);//우선수위 10으로 지정
		yt2.start();
		
		/*
		 종속 쓰레드 생성및 실행 (데몬쓰레드 실행)
		 */
		...
		System.out.println("현재호라성화된 상태위 쓰레드수"+ Thread.currentThread().getPriority());
